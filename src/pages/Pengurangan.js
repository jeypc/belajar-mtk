...
const Pengurangan = ({ navigation }) => {
  ...
  const [secondNumber, setSecondNumber] = useState(Math.floor(Math.random() * firstNumber) + 1);
  ...
  const handleAnswer = (value) => {
    if (value === correctAnswer) {
      ...
      const first = Math.floor(Math.random() * timesBy)
      setFirstNumber(first + 1)
      setSecondNumber(Math.floor(Math.random() * first) + 1)
    } else {
    ...
    <ImageBackground resizeMode={'cover'} style={{ width: null, height: null, flex: 1 }} source={require('./../../assets/img/page-bg.jpg')}>
      <SafeAreaView>
        <View style={{ alignContent: 'center', marginTop: 20, alignItems: 'center' }}>
          <Image source={require('./../../assets/img/pengurangan.png')} />
        </View>
